...
var g_database = {
    ...
    init: () => {
        ...
        for (var k of ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z', 1, 2, 3, 4, 5, 6, 7, 8, 9, 0]) {
            g_databases[k] = local_readJson('database_' + k, {});
        }
        ...
        registerAction('setFilter', (dom, action, params) => {
            ...
            g_database.loadItems(g_database.setFilter($(dom).attr('data-value')));
        });
        ...
        registerContextMenu('.sidebar-link[data-action="loadFolder"]', (dom, event) => {
        	...
        	var dialog = mobiscroll_($('#mobi_div').html(`
			    <div id="widget">
			        <div class="md-dialog">
			            <h4 data-action="folder_rename" ><i class="fa fa-pencil mr-10"aria-hidden="true"></i>
			                重命名</h4>
                            <h4 data-action="folder_setGroup" ><i class="fa fa-inbox mr-10"aria-hidden="true"></i>
                            <span>设置分组</span><span class="badge badge-pill badge-primary ml-10">0</span></h4>
			            <h4  data-action="folder_delete" class="text-danger"><i class="fa fa-trash-o mr-10" aria-hidden="true"></i>
			                删除</h4>
			        </div>
			    </div>
			    `), 'widget', {
    ...
    // 根据md5获取本地文件路径
    getLocalFile: (key) => {
    	var d = g_database.getImgData(key);
    ...
    // 获取所有为保存到本地的图片
    downloadUnsavedImages: () => {
        var res = {};
        for(var md5 of g_database.filterImage({"url": "data.i.startsWith('http')"})){
            res[md5] = g_database.getImgData(md5)['i'];
        }
         g_socket.send({
            type: 'downloadImages', 
            data: res
        ...
        // for(var md5 of g_database.filterImage({"url": "data.i.startsWith('data:image/')"})){
        //     res[md5] = g_database.getImgData(md5)['i'];
        // }
        //  g_socket.send({
        //     type: 'downloadImages', 
    ...
    saveImageToLocal: (opts, done) => {
     ...
     importImages: (datas, callback, filter = {}) => {
        ...
        for (var key in datas) {
        	i++;
            r.add(g_database.saveImgData(key, datas[key], false));
        }

        r.forEach((k) => {
        	local_saveJson('database_' + k, g_databases[k])
        });
        ...
        if(JSON.stringify(filter) == JSON.stringify(g_config.filter)) {
        	...
        	for (var md5 in datas) {
                var dom = $('.grid-item[data-md5="'+md5+'"]');
                if(dom.length){
                    dom.remove();
                }
        		h += g_gallery.getImageHtml(md5, datas[md5])
        	}
    ...
    importLocalImages: (datas) => {
    	...
    	for (var d of datas) {
            // if(!overwrite && g_database.getImgData(d.m)){
            //     continue;
            // }
           r[d.m] = {
            	i: d.f,
    ...
    selectImage: (md5) => {
        ...
        var d = g_database.getImgData(md5);
        if (d) {
    ...
    // 加载图片
    loadImage: (md5) => {
        var d = g_database.selectImage(md5);
        if (d) {
            $('#div_detail img').attr('src', g_database.getImageUrl(md5, d.i));
            $('#detail_link').val(d.r);

            g_cd.initElement(md5);
            loadImage(g_database.getImageUrl(md5, d.i, false));
        }
    },
    ...
    loadFolder: (key) => {
        if (g_folders[key]) {
            ...
            g_database.loadItems(g_database.setFilter({
                folder: key
            }));
        }
    },
    getImgsData: (keys, sort = 0) => {
        var r = {};
        var l = [];
        for (var md5 of keys) {
            r[md5] = g_database.getImgData(md5);
            l.push(md5);
        }
        switch(sort){
        	case 1: // 新到旧
    ...
    loadItems: (items) => {
        ...
        g_page.setList('gallery', {
            ...
            datas: g_database.getImgsData(items, 1),
            bottomHtml: `
    ...
    // 获取图片数据
    getImgData: (md5, clone = true) => {
        var k = md5.substr(0, 1).toLowerCase();
        if (g_databases[k][md5]) {
            var data = g_databases[k][md5];
    ...
    // 保存图片数据
    saveImgData: function(md5, data, update =true) {
        // _D('save', arguments);
        var k = md5.substr(0, 1).toLowerCase();
        g_databases[k][md5] = data;
        if(update) local_saveJson('database_' + k, g_databases[k]);
        return k;
    },
    ...
    filterImage: (filter) => {
        ...
        var imgs = new Set(filter['folder'] ? g_database.getFoldersImgs(filter['folder']) : g_database.getAllImgs())
        var opts = Object.keys(filter);
        for (var md5 of imgs.keys()) {
            var data = g_database.getImgData(md5);
            // eval判断，不符合条件的删除
            loop:
            for(var opt of opts){
                switch(opt){
    ...
    setFilter: function(filter) {
        ...
        local_saveJson('config', g_config);
        return g_database.filterImage(filter);
    ...
    getAllImgs: function() {
        var res = [];
        for (var key of Object.keys(g_databases)) {
            res = res.concat(Object.keys(g_databases[key]));
        }
        return res;
    },
